...
function incrementarPorUno(array) {
  ...
  // y devuelve el array
  // Tu código:
  let incrementados = array.map(function(x){
...
function cuentoElementos(arreglo){
  ...
  //Escribe tu código aquí
  let mayores = arreglo.filter(m => m > 18)
  ...
  var repetido = false;
    for (var i = 0; i < arreglo.length; i++) {
        for (var j = 0; j < arreglo.length; j++) {
            if (arreglo[i] == arreglo[j] && i != j) { //revisamos que i sea diferente de j, para que no compare el mismo elemento exacto.
                repetido= true;
             }
         }
     }
...
function mayorACien(array) {
  ...
  // Tu código:
   ...
   var filtrados = array.filter(mayores => mayores > 100);
  return filtrados;
}
